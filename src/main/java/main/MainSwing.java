...
public class MainSwing
{
	...
	private void verifyNoOtherInstancesAreRunning() 
	{
		if(!isFileshipAlreadyRunning()){
			JOptionPane.showMessageDialog(_frame,
				    ...
				    JOptionPane.INFORMATION_MESSAGE, IconLoader.transform);	
	...
	private static boolean isFileshipAlreadyRunning() 
	{
	    try 
	    {
	        final File file = new File("ASDM-LOCK.txt");
	...
	private void createTrayIcon() 
	{
		   ...
		   _frame.setDefaultCloseOperation(JFrame.HIDE_ON_CLOSE);
	        ...
	        final TrayIcon trayIcon =
	                new TrayIcon(IconLoader.tray_icon.getImage());
	        final SystemTray tray = SystemTray.getSystemTray();
	        trayIcon.setToolTip("Zhule's Meter");
	       
	        // Create a pop-up menu components
	        MenuItem aboutItem = new MenuItem("About");
	        CheckboxMenuItem cb1 = new CheckboxMenuItem("Show Notifications");
	...
	{		
		...
		if(ConfigFile.isSetup())
		{
			...
			JScrollPane pane = new JScrollPane(JScrollPane.VERTICAL_SCROLLBAR_NEVER, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
			...
			QuickHistoryLineScanner scanner = new QuickHistoryLineScanner();
			RecentHistoryParser parser = new RecentHistoryParser();
			...
			Runnable r = new Runnable() 
			{
				public void run() 
				{
					try 
					{
						List<String> currentLines = new LinkedList<String>();
	...
	private void buildAndShowUI() 
	{
		...
		JPanel artifactStage = new JPanel();
		...
		try
		{
			Image image = Toolkit.getDefaultToolkit().getImage(getClass().getResource("/transform_icon.jpg"));
	...
	public void moveBarToInactive(TransformBar transformBar) 
	{
		System.out.println("Transferring xform from live to dead: " + transformBar.getPlayerName());
		_xformStage.transformDied(transformBar);
		int newCount = _xformStage.getNumActiveXforms();
		updateXformTab(newCount);
    	_stage.repaint();
	}


	public synchronized void transformDetected(PlayerData data) 
	{
	...
	public synchronized void removeCooldown(TransformBar transformBar) 
	{
	...
	public synchronized void checkDeath(PlayerData data) 
	{
	...
	private synchronized void updateXformTab(int newCount) 
	{
	    int count = _tabbedPane.getTabCount();
	    for (int i = 0; i < count; i++) 
	    {
	      String label = _tabbedPane.getTitleAt(i);
	...
	public synchronized void artifactWasCaptured(ArtifactData info) 
	{
