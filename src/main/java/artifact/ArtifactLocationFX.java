...
public class ArtifactLocationFX extends VBox 
{
	...
	private Label legionLabl;
	private Label vulnLabel;
	private Label triggerLabel;
	private Label noData;
	
	private VBox activeData = new VBox();
	private AbyssArtifact _artie;
	private ArtifactData _lastCaptureData;

	public ArtifactLocationFX(AbyssArtifact artie) 
	{
		...
		triggerLabel = makeLabel("", Color.GREEN, 20, new ImageView(IconLoader.loadFxImage("artifact-fire.png", 32)));
	...
	protected void artieCountdownTimer()
	{
		try 
		{
			if (_lastCaptureData != null)
			{
				...
				double scale = ((double) vulnerableCooldown
				...
				{
					useableCooldown = 0;
					triggerLabel.setTextFill(Color.GREEN);
	...
	public void artifactCaptured(ArtifactData info) 
	{
		...
		{
			factionLabel.setTextFill(Color.GREEN);
			legionLabl.setTextFill(Color.GREEN);
		} 
		else 
		{
	...
	private Label makeLabel(String txt, Color color, int size, Node icon) {
		Label label = new Label(txt, icon);
		label.setTextFill(color);
		label.setFont(Font.font(size));
		return label;
	}

	private Label makeLabel(String txt, Color color, int size) {
