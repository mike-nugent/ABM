...
public class IconLoader {
	...
	private static ImageIcon loadIcon(Archetype cz) {
		try 
		{
			InputStream in = IconLoader.class.getResourceAsStream("/"+cz.getIconName()); 
			
			return new ImageIcon(ImageIO.read(in).getScaledInstance(-1,
	...
	public static javafx.scene.image.Image loadFxImage(String icon) 
	{
		return loadFxImage(icon, ICON_SIZE);
	}
	
	
	public static javafx.scene.image.Image loadFxImage(String icon, int scaleHeight) {
		try 
		{
			InputStream in = IconLoader.class.getResourceAsStream("/"+icon); 
	...
	private static ImageIcon loadSpecial(String icon, int scaleHeight) {
		try 
		{
			InputStream in = IconLoader.class.getResourceAsStream("/"+icon); 
	...
	public static ImageIcon getIcon(Archetype cz) 
	{
		switch (cz) 
		{
			case Aethertech:
